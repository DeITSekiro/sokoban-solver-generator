...
def play_game(window, level=1, random_game=False, random_seed=None, **widgets):
    ...
    while game_loop:
        ...
        for event in events:
            ...
            elif event.type == SOLVE_GREEDY_EVENT:
                print('Finding a solution for the puzzle\n')
                widgets['paths'].reset('Solving with [Greedy]')
                show_solution = True
                start = time.time()
                solution, depth = solve_greedy(
                    game.get_matrix(), 
                    widget=widgets['paths'], 
                    visualizer=widgets['toggle'].getValue(),
                )
...
def main():
    ...
    background_image = pygame.image.load('img/sokoban.png')
